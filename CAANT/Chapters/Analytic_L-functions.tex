\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Analytic Data}
    ...
    \begin{enumerate}[align=left]
      ...
      \item[\textbf{Euler product:}] For every prime \(p\), there exist complex numbers \((\a_{j}(p))_{1 \le j \le d_{L}}\) called the \textbf{Euler parameters}\index{Euler parameters} of \(L(s)\) at \(p\) satisfying \(|\a_{j}(p)| \le p^{\theta}\) for some \(0 \le \theta < 1\) and are such that \(L(s)\) admits the Euler product
      \[
