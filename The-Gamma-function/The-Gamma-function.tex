...
\begin{document}
  ...
  \section{Asymptotics}
    ...
    We now turn to estimates for the gamma function in vertical strips. That is, when \(\s\) is bounded. The first of which is useful asymptotic equivalence showing that the gamma function decays exponentially in \(t\).
    
    \begin{corollary}\label{equ:weaker_Stirling_formula_1}
      Let \(|\arg(s)| < \pi-\e\) and \(|s| > \d\) for some \(\e,\d > 0\) and suppose \(\s_{1} \le \s \le \s_{2}\). Then
      \[
    ...
    \begin{corollary}\label{equ:weaker_Stirling_formula_2}
      ...
      In particular,
      \[
        \frac{\G(1-s)}{\G(s)} \ll_{\e} |t|^{1-2\s}.
      \]
      All of these estimates hold with \(|t|+3\) replacing \(|t|\) if we merely assume \(s\) is at least distance \(\e\) away from the poles of the gamma function in this vertical strip.
    \end{corollary}
    \begin{proof}
    ...
    The choice of \(3\) in \(|t|+3\) at the end of \cref{equ:weaker_Stirling_formula_2} is a matter of convenience as could use any positive constant. In particular, it is useful when taking logarithms as \(\ln(|t|+3) \ge 1\).
    ...
    \begin{corollary}\label{equ:weaker_approximtion_for_digamma_2}
      ...
      \[
        \frac{\G'}{\G}(s) = \log|s|.
      \]
      This estimate holds with \(|t|+3\) replacing \(|t|\) if we merely assume \(s\) is at least distance \(\e\) away from the poles of the gamma function in this vertical strip.
    \end{corollary}
    \begin{proof}
      \cref{equ:weaker_approximtion_for_digamma_1} implies
      \[
        \frac{\G'}{\G}(s) \ll_{\e} \log|s|.
      \]
      which is a strictly weaker estimate. Since the gamma function is holomorphic and bounded on the compact set defined by \(\s_{1} \le \s \le \s_{2}\) and \(|t| \le \e\) with \(s\) at least distance \(\e\) away from the poles, it is absolutely bounded. The second statement follows.
    \end{proof}
\end{document}