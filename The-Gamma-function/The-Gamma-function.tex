...
\begin{document}
  ...
  \section{Analytic Properties}
    Throughout \(s = \s+it\) and \(u = \tau+ir\) will stand for complex variables with \(\s,\tau,t,r\) real. The \textit{gamma function} \(\G(s)\) is defined by the integral
    \[
    ...
    \end{proposition}
    \begin{proof}
      The first two identities follow by direct computation with the second requiring an application of integration by parts. The following statment is obtained by repeated application of the latter identity. The last statement follows from the identity theorem and that \(\G(s)\) is real for \(\s > 1\).
    \end{proof}
    ...
    \[
      \G(s+1) = s\G(s),
    \]
    is referred to as the \textit{functional equation} for the gamma function. The functional equation will allow for the meromorphic conduction of the Gamam function to all of \(\C\).
    ...
    Importat resisdues are
    \[
      \Res_{s = 0}\G(s) = 1 \quad \text{and} \quad \Res_{s = 1}\G(s) = -1.
    \]
    In order to deduce more analytic properties of the gamma function, we will introduce an auxillary function that will be more amenable to study. The \textit{beta function} \(B(s,u)\) is defined by
    \[
    ...
    \[
        B(s,u) = B(u,s),
    \]
    as it seen by performing the change of varibles \(x \mapsto 1-x\) to the integral. The following result establishes the relationship between the beta function and the gamma function:
    ...
    It it not yet possible to meromorphically continue the beta function to \(\C^{2}\) because we cannot divide by the gamma function as we do not yet know where it possess zeros. It turns out that the gamma function has no zeros and we will prove this shortly. The essential result is to prove Euler's reflection formula which relates the gamma function to a periodic function of which we know its zeros explicitly. To acomplish this we will make use of \cref{prop:integral_reprepsentation_for_beta_function} and a formula for the beta function wich will allow us to compute it by means of the residue theorem. To deduce this formula, apply the change of variables \(x \mapsto \frac{x}{x+1}\) to the beta function to obtain
    \begin{equation}\label{equ:residue_form_beta_function}
      B(s,u) = \int_{0}^{\infty}\frac{x^{s-1}}{(x+1)^{s+u}}\,dx.
    \end{equation}
    This shows that beta function is the Mellin transform of \(\frac{1}{(s+1)^{s+u}}\). This formula is immensely useful when \(s+u = 1\) because we may then evaulate \(B(s,u)\) by means of the residue theorem. We will make use of this fact to prove \textit{Euler's reflection formula}:
    ...
    \begin{proof}
      ...
      \]
      The arugment of \(s\) along \(\eta_{3}\) is \(2\pi\) that of it along \(\eta_{1}\). Therefore
      \[
      ...
      \]
      Similarily, the parameterization \(z \mapsto \e e^{i\t}\) for \(\eta_{4}\) shows that the integral along this contour tends to zero as \(\e \to 0\) and is independent of \(R\). Hence 
      \[
    ...
    \end{proof}   
  \section{Aymptotics}
    Upon inspection of the Hamard factorization, we find that the reciprocal of the gamma function is of order \(1\). We now turn to analyzing the growth rate of the gamma function itself. In fact, we can obtain a well-known asymptotic formula for the gamma function known as \textit{Stirling's formula}:
    ...
    \begin{proof}
      ...
      \]
      The change of variablex \(z \to s(1+z)\) gives
      \[
      ...
      \]
      Use this estimat in \(I_{1}(s)\) to write
      \[
      ...
      \]
      where we have used \cref{equ:gamma_function_at_1/2} to evaulate the gamma function. By the exponential decay of the integrand, we have say
      \[
    ...
    We now turn to bounds for the gamam function in vertical strips. That is, when \(\s\) is bounded. The first of which is useful asymptotic equivalence showing that the gamam function decays exponentially in \(t\).
    ...
    Notice that \cref{equ:weaker_Stirling_formula_1} is not valid on the negative real axis away from the poles of the gamma function. We can remedy this provided we weaken the asymptotic slighly.
    ...
    \begin{corollary}\label{equ:weaker_Stirling_formula_2}
      ...
      \]
      All of these estimates hold with \(|t|+1\) replacing \(|t|\) if we merely assume \(s\) is distance \(\e\) away from the poles of the gamma function in this vertical strip.
    \end{corollary}
    \begin{proof}
    ...
    \end{corollary}
    \begin{proof}
      As the gamam function has no zeros, we may use its functional equation to write
      \[
