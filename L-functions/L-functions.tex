...
\begin{document}
  ...
  \section{Analytic Data}
    ...
    \begin{enumerate}[align=left]
      ...
      \]
      This is called the \textit{functional equation} of \(L(s)\). The tuple \((\e,Q,\mu_{1},\ldots,\mu_{d})\) is called the \textit{functional equation data} of \(L(s)\). If \(p \mid Q\) we say \(p\) is \textit{ramified} while if \(p \nmid Q\) we say \(p\) is \textit{unramified}.
      ...
      \[
        L_{p}(T) = (1-\a_{1}(p)T) \cdots (1-\a_{d}(p)T),
      \]
      is called the \textit{local factor} of \(L(s)\) at \(p\). If \(p\) is unramified then \(L_{p}\) is of degree \(d\) while if \(p\) is ramified then \(L_{p}\) is of degree less than \(d\).
    \end{enumerate}
    ...
    \begin{enumerate}[label*=(\roman*)]
      ...
      \item If we merely assume \((s-1)^{r}L(s)\) is finite order then the functional equation forces the order to be \(1\). This can be deduced by considering the entire function
    \[
      s^{r}(s-1)^{r}\L(s),
    \]
    and applying the Phragm\'en-Lindel\"of convexity principle in vertical strips.
    \item The condition that the root number satisfies \(|\e| = 1\) isn't strictly necessary. For applying the functional equation twice shows \(|\e|^{2} = 1\).
    ...
    \[
      a_{L}(p^{k}) = \sum_{1 \le j_{1} \le \cdots \le j_{k} \le d}\a_{j_{1}}(p) \cdots \a_{j_{k}}(p).
    \]
    In other words, \(a_{L}(p^{k})\) is the complete symmetric polynomial of degree \(k\) in the local parameters at \(p\) and there are \(\binom{k+d-1}{d-1} \ll \s_{d}(p^{k})\) many terms. If the Ramanujan bound holds, then it follows that \(a_{L}(n) \ll \s_{d}(n)\). This implies the slightly weaker, but more practical, estimate \(a_{L}(n) \ll_{\e} n^{\e}\).
    \end{enumerate}
    ...
    \]
    The choice of \(3\) in \(|s+\mu_{j}|+3\) is a matter of convenience as could use any positive constant. In particular, it is useful when taking logarithms as \(\ln(|s+\mu_{j}|+3) \ge 1\). For legibility, we write also write
    \[
    ...
    provided \(\s_{1} \le \s \le \s_{2}\) and \(s\) is at least distance \(\e\) away from the poles of the gamma function. Then the estimates
     \begin{equation}\label{equ:gamma_factor_analytic_conductor_estimate}
      \frac{L_{\infty}(1-s)}{L_{\infty}(s)} \ll_{\e} \mf{q}_{\infty}(s)^{\frac{1}{2}-\s} \quad \text{and} \quad Q^{\frac{1}{2}-s}\frac{L_{\infty}(1-s)}{L_{\infty}(s)} \ll_{\e} \mf{q}(s)^{\frac{1}{2}-\s},
    \end{equation}
    ...
    \begin{equation}\label{equ:digamma_gamma_factor_analytic_conductor_estimate}
      \frac{L_{\infty}'}{L_{\infty}}(s) \ll_{\e} \log\mf{q}_{\infty}(s) \quad \text{and} \quad \log{Q}+\frac{L_{\infty}'}{L_{\infty}}(s) \ll_{\e} \log\mf{q}(s),
    \end{equation}
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{theorem*}[Approximate functional equation]
      ...
      \[
        L(s) = \sum_{n \ge 1}\frac{a(n)}{n^{s}}V_{s}\left(\frac{n}{\sqrt{Q}X}\right)+\e(s)\sum_{n \ge 1}\frac{\conj{a(n)}}{n^{1-s}}V_{1-s}\left(\frac{nX}{\sqrt{Q}}\right)+\frac{R(s,X,f)}{Q^{\frac{s}{2}}L_{\infty}(s)},
      \]
      where \(V_{s}(y)\) is the inverse Mellin transform defined by
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{L_{\infty}(s)}\Phi(u)y^{-u}\frac{du}{u},
      \]
      and
      \[
        \e(s) = \e(f)Q^{\frac{1}{2}-s}\frac{L_{\infty}(1-s)}{L_{\infty}(s)}.
      \]
    ...
    \begin{proof}
      ...
      \begin{align*}
        I(X,s,f) &= \frac{1}{2\pi i}\int_{(a)}\L(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} \\
        &= \frac{1}{2\pi i}\int_{(a)}Q^{\frac{s+u}{2}}\g(s+u,f)L(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} \\
        &= \frac{1}{2\pi i}\int_{(a)}\sum_{n \ge 1}\frac{a(n)}{n^{s+u}}Q^{\frac{s+u}{2}}\g(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} \\
        &= \sum_{n \ge 1}\frac{1}{2\pi i}\int_{(a)}\frac{a(n)}{n^{s+u}}Q^{\frac{s+u}{2}}\g(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} && \text{FTT} \\
        &= Q^{\frac{s}{2}}L_{\infty}(s)\sum_{n \ge 1}\frac{a(n)}{n^{s}}\frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{L_{\infty}(s)}\Phi(u)\left(\frac{\sqrt{Q}X}{n}\right)^{u}\,\frac{du}{u} \\
        &= Q^{\frac{s}{2}}L_{\infty}(s)\sum_{n \ge 1}\frac{a(n)}{n^{s}}V_{s}\left(\frac{n}{\sqrt{Q}X}\right).
      \end{align*}
      Performing the same computation for \(I(X^{-1},1-s,\conj{f})\) and substituting in the results, we arrive at
      \begin{align*}
        \L(s) &= Q^{\frac{s}{2}}L_{\infty}(s)\sum_{n \ge 1}\frac{a(n)}{n^{s}}V_{s}\left(\frac{n}{\sqrt{Q}X}\right) \\
        &+ \e(f)Q^{\frac{1-s}{2}}L_{\infty}(1-s)\sum_{n \ge 1}\frac{\conj{a(n)}}{n^{1-s}}V_{1-s}\left(\frac{nX}{\sqrt{Q}}\right)+R(s,X,f).
      \end{align*}
      Diving by \(Q^{\frac{s}{2}}L_{\infty}(s)\) completes the proof.
    \end{proof}
    ...
    \begin{proposition}\label{prop:V_function_decay}
      ...
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{(2M-2)}\frac{\g(s+u,f)}{L_{\infty}(s)}\Phi(u)y^{-u}\frac{du}{u}.
      \]
    ...
    \begin{proof}
      ...
      \[
        \frac{\g(s+u,f)}{L_{\infty}(s)} \ll \mf{q}_{\infty}(s)^{\frac{\tau}{2}}e^{d_{f}\frac{\pi}{2}|r|},
      \]
      ...
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{(M)}\frac{\g(s+u,f)}{L_{\infty}(s)}\Phi(u)y^{-u}\frac{du}{u} \\
      \]
    ...
    \begin{theorem}
      ...
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{\e(f)}{\sqrt{Q}}\sum_{n \ge 1}\conj{a(n)}\phi(n)+R(f)\Psi(1),
      \]
      where \(\phi(y)\) is the inverse Mellin transform defined by
      \[
        \phi(y) = \frac{1}{2\pi i}\int_{(a)}Q^{s}\frac{L_{\infty}(s)}{L_{\infty}(1-s)}y^{-s}\Psi(1-s)\,ds,
      \]
    ...
    \begin{proof}
      ...
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{1}{2\pi i}\int_{(1-a)}\e(f)Q^{\frac{1}{2}-s}\frac{L_{\infty}(1-s)}{L_{\infty}(s)}L(1-s,\conj{f})\Psi(s)\,ds+R(f)\Psi(1).
      \]
      Performing the change of variables \(s \mapsto 1-s\) in this latter integral gives
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{1}{2\pi i}\int_{(a)}\e(f)Q^{s-\frac{1}{2}}\frac{L_{\infty}(s)}{L_{\infty}(1-s)}L(s,\conj{f})\Psi(1-s)\,ds+R(f)\Psi(1).
      \]
      The proof is complete upon interchanging the sum over the Dirichlet series and the integral by the Fubini–Tonelli theorem and factoring out \(\frac{\e(f)}{\sqrt{Q}}\).
    \end{proof}
  \section{The Riemann Hypothesis and Nontrivial Zeros}
    ...
    \begin{equation}\label{equ:fun_eq_for_zeros}
      L(s) = \e(f)Q^{\frac{1}{2}-s}\frac{L_{\infty}(1-s)}{L_{\infty}(s)}L(1-s,\conj{f}).
    \end{equation}
  ...
  \section{The Lindel\"of Hypothesis and Convexity Bounds}
    ...
    \[
      L(s) = \e(f)Q^{\frac{1}{2}-s}\frac{L_{\infty}(1-s)}{L_{\infty}(s)}L(1-s,f).
    \]
