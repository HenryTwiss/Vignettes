...
\begin{document}
  \date{}
  \maketitle
  Throughout, \(s = \s+it\) and \(u = \tau+ir\) will stand for complex variables with \(\s\), \(\tau\), \(t\), and \(r\) real. We also set
  \[
    \G_{\R}(s) = \pi^{-\frac{s}{2}}\G\left(\frac{s}{2}\right) \quad \text{and} \quad \G_{\C}(s) = (2\pi)^{-s}\G(s).
  \]
  In this notation, Legendre's duplication formula takes the form
  \[
    \G_{\C}(s) = \G_{\R}(s)\G_{\R}(s+1).
  \]
  \section{Analytic Data}
    ...
    \]
    with coefficients \(a(n) \in \C\) satisfying the following properties:
    \begin{enumerate}[align=left]
      \item[\textbf{Analyticity:}] There is an nonnegative integer \(r\) such that \(L(s)\) is absolutely convergent for \(\s > 1\) and admits meromorphic continuation to \(\C\) with at most a pole at \(s = 1\) of order \(r\) such that \((s-1)^{r}L(s)\) is of order \(1\).
      \item[\textbf{Functional Equation:}] There exists a positive integer \(Q\), called the \textit{conductor} of \(L\), a positive integer \(d\) called the \textit{degree} of \(L\), a pair of nonnegative integers \((d_{1},d_{2})\) called the \textit{signature} of \(L\) with \(d = d_{1}+d_{2}\), and complex numbers \((\mu_{j})_{1 \le j \le d_{1}}\) and \((\nu_{k})_{1 \le k \le d_{2}}\) called the \textit{spectral parameters} of \(L\) whose imaginary parts are all larger than \(-1\), all of which determine functions
      \[
        \L(s) = Q^{\frac{s}{2}}L_{\infty}(s)L(s) \quad \text{and} \quad L_{\infty}(s) = \prod_{j}\G_{\R}(s+\mu_{j})\prod_{k}\G_{\C}(s+\nu_{k}),
      \]
      called the \textit{completion} of \(L\) and the \textit{archimedean factor} of \(L\) respectively, such that the completion has the following properties:
      \begin{enumerate}[align=left]
        \item[\textbf{Bounded in vertical strips:}] \(\L(s)\) is polynomially bounded in vertical strips away from its poles. That is, for \(\s_{1} \le \s \le \s_{2}\) there is a positive constant \(A(\s_{1},\s_{2})\) such that
        \[
          \L(s) \ll_{s_{1},\s_{2},\e} \ll (|t|+1)^{A(\s_{1},\s_{2})},
        \]
        provided \(s\) is at least distance \(\e\) away from the poles of \(\L(s)\) in this vertical strip.
        \item[\textbf{Functional equation:}] There exists a \(\e \in \C\), called the \textit{sign} of \(L\), such that
        \[
          \L(s) = \e\conj{\L(1-\conj{s})}.
        \]
      \end{enumerate}
      \item[\textbf{Euler product:}] For every prime \(p\), there exist a polynomial \(L_{p} \in \C(T)\) called the \textit{local factor} of \(L\) at \(p\) of degree \(d_{p} \le d\) with roots \((\a_{\ell,p})_{1 \le \ell \le d_{p}}\) called the \textit{Satake parameters} of \(L\) at \(p\), such that \(L\) admits the Euler product
      \[
        L(s) = \prod_{p}L_{p}(p^{-s})^{-1}.
      \]
      Moreover, if \(p \nmid Q\) then \(d_{p} = d\) and if \(p \mid Q\) then \(d_{p} < d\). In the former case \(p\) is said to be \textit{ramified} and in the latter case \(p\) is said to be \textit{unramified}.
      \item[\textbf{Central character:}] There exists a Dirichlet character \(\chi\) of conductor dividing \(Q\) called the \textit{central character} of \(L\) such that has the following properties:
      \begin{enumerate}[align=left]
        \item[\textbf{Leading term:}] For every prime \(p\), the local factor at \(p\) is of the form
        \[
          L_{p}(T) = 1-a_{p}T+\cdots+(-1)^{d}\chi(p)T^{d}.
        \]
        \item[\textbf{Balanced:}] We have
        \[
          \Im\left(\sum_{j}\mu_{j}+\sum_{k}(2\nu_{k}+1)\right) = 0.
        \]
        That is, the spectral parameters are either real or occur in conjugate pairs.
        \item[\textbf{Parity:}] The spectral parameters determine the parity of the central character via
        \[
          \chi(-1) = (-1)^{\sum_{j}\mu_{j}+\sum_{k}(2\nu_{k}+1)}.
        \]
      \end{enumerate}
    \end{enumerate}
      
    \iffalse
      The \textit\{analytic conductor\} \(\mf{q}(s)\) is defined to be
      \[
        \mf{q}(s) = q(f)\mf{q}_{\infty}(s),
      \]
      where we set
      \[
        \mf{q}_{\infty}(s) = \prod_{1 \le j \le d_{f}}(|s+\k_{j}|+3).
      \]
      ...
      \]
      \item The \textit\{completed \(L\)-function\}
      \[
        \L(s) = q(f)^{\frac{s}{2}}\g(s)L(s),
      \]
      must admit meromorphic continuation to \(\C\) with at most poles at \(s = 0\) and \(s = 1\). Moreover, it must satisfy the \textit\{functional equation\} given by
      \[
        \L(s) = \e(f)\L(1-s,\conj{f}),
      \]
      where \(\e(f)\) is a complex number with \(|\e(f)| = 1\) called the \textit\{root number\} of \(L(s)\), and \(\conj{f}\) is an object associated to \(f\) called the \textit\{dual\} of \(f\) such that \(L(s,\conj{f})\) satisfies \(a_{\conj{f}}(n) = \conj{a(n)}\), \(\g(s,\conj{f}) = \g(s)\), \(q(\conj{f}) = q(f)\), and \(\e(\conj{f}) = \conj{\e(f)}\). We call \(L(s,\conj{f})\) the \textit\{dual\} of \(L(s)\). If \(\conj{f} = f\), we say \(L(s)\) is \textit\{self-dual\}.
      \item \(L(s)\) admits meromorphic continuation to \(\C\) with at most a pole at \(s = 1\) of order \(r_{f} \in \Z\), and must be of order \(1\) (see \cref{append:Factorizations_and_Finite_Order}) after clearing the possible polar divisor.
    \end{enumerate}
    Property (ii) ensures that \(\g(s)\) is holomorphic and nonzero for \(\s \ge 1\). Then as \(\g(1,f)\) is nonzero, \(r_{f}\) is also the order of possible poles of \(\L(s)\) at \(s = 0\) and \(s = 1\) which are equal by the functional equation. It follows that \(r_{\conj{f}} = -r_{f}\). As \(L(s)\) admits meromorphic continuation by property (v), we denote the continuation by \(L(s)\) as well. Also note that the product of \(L\)-functions is an \(L\)-function but the sum of \(L\)-functions need not be.
    ...
    We say that an \(L\)-function \(L(s)\) belongs to the \textit\{Selberg class\} if the additional adjustments to (i), (ii), and (v) hold:
    \begin{itemize}
      ...
      \item[(v)] \(L(s)\) has at most a simple pole at \(s = 1\). That is, \(r_{f} \le 1\).
    \end{itemize}
    The adjustment for (i) is called the \textit\{generalized Ramanujan-Petersson conjecture\} and it forces \(a(n) \ll \s_{0}(n) \ll_{\e} n^{\e}\) (recall \cref{prop:sum_of_divisors_growth_rate}). The adjustment for (ii) is called the \textit\{generalized Selberg conjecture\} and it ensures that \(\g(s)\) is holomorphic and nonzero for \(\s > 0\). As for the adjustment for (v), it is expected that \(L(s)\) is entire unless \(\a_{i}(p) \ge 0\). The Selberg class constitutes a very special class of \(L\)-functions. Note that the product of two Selberg class \(L\)-functions is also Selberg class. Accordingly, we say that a Selberg class \(L\)-function is \textit\{primitive\} if it cannot be factored into a product of two Selberg class \(L\)-functions of strictly smaller positive degree (positive degree is necessary so that one of the factors cannot be \(L(s,\mathbf{1})\)). Clearly, any Selberg class \(L\)-function of degree \(1\) is primitive.

    \begin{remark}
      In general, it is a difficult problem to determine when an \(L\)-function \(L(s)\) of degree \(d_{f} \ge 2\) is primitive.
    \end{remark}

    Let \(L(s)\) be an \(L\)-function with local roots \(\a_{j}(p)\) at \(p\) and let \(k \ge 1\). We say that an \(L\)-function \(L(s,\sym^{k}f)\) of degree \(\binom{d_{f}+k-1}{k}\) is the \(k\)-th \textit\{symmetric power\} of \(L(s)\) if it satisfies the following adjustment:
    \begin{itemize}
      ...
      \]
    \end{itemize}
    When \(k\) is \(2\), \(3\), etc.\ we simply refer to \(L(s,\sym^{k}f)\) as the \textit\{symmetric square\}, \textit\{symmetric cube\}, etc. When \(k \le d_{f}\), we say that an \(L\)-function \(L(s,\ext^{k}f)\) of degree \(\binom{d_{f}}{k}\) is the \(k\)-th \textit\{exterior power\} of \(L(s)\) if it satisfies the following adjustment:
    \begin{itemize}
      ...
      \]
    \end{itemize}
    When \(k\) is \(2\), \(3\), etc.\ we simply refer to \(L(s,\ext^{k}f)\) as the \textit\{exterior square\}, \textit\{exterior cube\}, etc.

    \begin{remark}
      It is not an easy matter to determine if the \(k\)-th symmetric power and \(k\)-th exterior power \(L\)-functions of \(L(s)\) exist.
    \end{remark}

    Now suppose we are given two \(L\)-functions \(L(s)\) and \(L(s,g)\) with local roots \(\a_{j}(p)\) and \(\b_{\ell}(p)\) at \(p\). We say that an \(L\)-function \(L(s,f \ox g)\) of degree \(d_{f \ox g} = d_{f}d_{g}\) is the \textit\{Rankin-Selberg convolution\} of \(L(s)\) and \(L(s,g)\) (or \textit\{Rankin-Selberg square\} if \(f = g\)) if it satisfies the following adjustment:
    \begin{itemize}
    ...
    \begin{equation}\label{equ:gamma_estimates}
    ...
    \end{equation}
    are valid in the vertical strip \(a \le \s \le b\) provided \(s\) is distance \(\e\) away from the poles of \(\G(s)\) in the former case. Clearly \cref{equ:modified_gamma_estimates} implies \cref{equ:gamma_estimates} when \(|t| > 1\). In any case, we immediately obtain the useful estimate
    \[
      \frac{\G(1-s)}{\G(s)} \ll_{\e} (|t|+1)^{1-2\s},
    \]
    valid in the vertical strip \(a \le \s \le b\) provided \(s\) is distance \(\e\) away from the poles of \(\G(1-s)\). Moreover, it easily follows from the definition of \(\g(s)\) that the estimates
    \begin{equation}\label{equ:gamma_factor_analytic_conductor_estimate}
      \frac{\g(1-s,f)}{\g(s)} \ll_{\e} \mf{q}_{\infty}(s)^{\frac{1}{2}-\s} \quad \text{and} \quad q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s)} \ll_{\e} \mf{q}(s)^{\frac{1}{2}-\s},
    \end{equation}
    are valid in the vertical strip \(a \le \s \le b\) provided \(s\) is distance \(\e\) away from the poles of \(\g(1-s,f)\). There are analogous useful estimates for the digamma function as well. To derive them, suppose \(\s\) is bounded and \(|t| > 1\). Then \(s\) is bounded away from zero, so making use of the formula \(\frac{\G'}{\G}(s+1) = \frac{\G'}{\G}(s)+\frac{1}{s}\) if \(\s < 0\), \cref{equ:approximtion_for_digamma} together with the fact \(\log(s) = \log|s|+i\arg(s)\) gives
    \begin{equation}\label{equ:digamma_estimate}
      \frac{\G'}{\G}(s) \ll \log|s|.
    \end{equation}
    We can also obtain estimates that holds in vertical strips. Suppose \(s\) is in the vertical strip \(a \le \s \le b\) and is distance \(\e\) away from the poles of \(\G(s)\). Then \(\frac{\G'}{\G}(s)\) is bounded on the compact region \(a \le \s \le b\) with \(|t| \le 1\) provided \(s\) is distance \(\e\) away from the poles of \(\G(s)\). Then the estimate
    \begin{equation}\label{equ:modified_digamma_estimate}
      \frac{\G'}{\G}(s) \ll_{\e} \log(|s|+1),
    \end{equation}
    is valid in the vertical strip \(a \le \s \le b\) provided \(s\) is distance \(\e\) away from the poles of \(\G(s)\). Clearly \cref{equ:modified_digamma_estimate} implies \cref{equ:digamma_estimate} when \(|t| > 1\). It follows from the definition of \(\g(s)\) that the estimates
    \begin{equation}\label{equ:digamma_gamma_factor_analytic_conductor_estimate}
      \frac{\g'}{\g}(s) \ll_{\e} \log\mf{q}_{\infty}(s) \quad \text{and} \quad \log{q(f)}+\frac{\g'}{\g}(s) \ll_{\e} \log\mf{q}(s),
    \end{equation}
    are valid in the vertical strip \(a \le \s \le b\) provided \(s\) is distance \(\e\) away from the poles of \(\g(s)\). Also, our definition of \(\mf{q}_{\infty}(s)\) ensures
    \[
      \log\mf{q}(s) \ge \log\mf{q}(f) \ge \log(3) \ge 1.
    \]
  \section{The Approximate Functional Equation}
    If \(L(s)\) is an \(L\)-function then there is a formula which acts as a compromise between the functional equation for \(L(s)\) and expressing \(L(s)\) as a Dirichlet series. This formula is known as the approximate functional equation and it is important because it is valid inside of the critical strip and therefore can be used to obtain data about \(L(s)\) in that region. First we use \cref{equ:gamma_factor_analytic_conductor_estimate} to show that \(L(s)\) has polynomial growth in the \(t\)-aspect in vertical strips:

    \begin{proposition}\label{prop:L_function_bounded_in_vertical_strips}
      For any \(L\)-function \(L(s)\) and \(a < b\), \(L(s)\) is of polynomial growth in the \(t\)-aspect in the vertical half-strip \(a \le \s \le b\) with \(|t| \ge 1\).
    \end{proposition}
    \begin{proof}
      Observe that \(L(s) \ll 1\) on the half-line \(\s = \max(1,b)+\e\) with \(t \ge 1\). Now the functional equation and \cref{equ:gamma_factor_analytic_conductor_estimate} together imply
      \[
        L(s) \ll \mf{q}(s)^{\frac{1}{2}-\s}L(1-s,f).
      \]
      Then \(L(s)\) is also of polynomial growth in the \(t\)-aspect on the half-line \(\s \le \min(0,a)-\e\) with \(t \ge 1\) since \(\mf{q}(s)^{\frac{1}{2}-\s}\) does and \(L(1-s,f) \ll 1\). Moreover, the estimate above also implies that \(L(s)\) is bounded on the line \(t = 1\) for \(\min(0,a)-\e \le \s \le \max(1,b)+\e\). As \(L(s)\) is holomorphic for \(|t| \ge 1\) and of order \(1\), we can apply the Phragm\'en-Lindel\"of convexity principle in this region (see \cref{append:The_Phragmen_Lindelof_Convexity_principle}) so that \(L(s)\) is of polynomial growth in the \(t\)-aspect in the vertical half-strip \(a \le \s \le b\) with \(|t| \ge 1\).
    \end{proof}

    We now prove the \textit\{approximate function equation\}:

    \begin{theorem*}[Approximate functional equation]
      Let \(L(s)\) be an \(L\)-function, \(\Phi(u)\) be an even holomorphic function bounded in the vertical strip \(|\tau| < a+1\) for any \(a > 1\) such that \(\Phi(0) = 1\), and let \(X > 0\). Then for \(s\) in the critical strip, we have
      \[
        L(s) = \sum_{n \ge 1}\frac{a(n)}{n^{s}}V_{s}\left(\frac{n}{\sqrt{q(f)}X}\right)+\e(s)\sum_{n \ge 1}\frac{\conj{a(n)}}{n^{1-s}}V_{1-s}\left(\frac{nX}{\sqrt{q(f)}}\right)+\frac{R(s,X,f)}{q(f)^{\frac{s}{2}}\g(s)},
      \]
      where \(V_{s}(y)\) is the inverse Mellin transform defined by
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s)}\Phi(u)y^{-u}\frac{du}{u},
      \]
      and
      \[
        \e(s) = \e(f)q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s)}.
      \]
      Moreover, \(R(s,X,f)\) is zero if \(\L(s)\) is entire, and otherwise
      \[
    ...
    \begin{proof}
      ...
      \]
      \(L(s)\) has polynomial growth in the \(t\)-aspect by \cref{prop:L_function_bounded_in_vertical_strips}. From \cref{equ:modified_gamma_estimates} we see that \(\g(s+u,f)\) exhibits exponential decay. Since \(\Phi(u)\) is bounded, it follows that the integrand exhibits exponential decay in a vertical strip containing \(|\tau| \le a\). Therefore the integral is locally absolutely uniformly convergent. Moreover, we may shift the line of integration to \((-a)\). In doing so, we pass by a simple pole at \(u = 0\) and possible poles at \(u = 1-s\) and \(u = -s\), giving
      \[
        I(X,s,f) = \frac{1}{2\pi i}\int_{(-a)}\L(s+u,f)\Phi(u)X^{u}\,\frac{du}{u}+\L(s)+R(s,X,f).
      \]
      Applying the functional equation to \(\L(s+u,f)\) and performing the change of variables \(u \mapsto -u\), we obtain
      \[
        I(X,s,f) = -\e(f)I(X^{-1},1-s,\conj{f})+\L(s)-R(s,X,f),
      \]
      since \(\Phi(u)\) is even. This equation is equivalent to
      \[
        \L(s) = I(X,s,f)+\e(f)I(X^{-1},1-s,\conj{f})+R(s,X,f).
      \]
      Since \(\Re(s+u) > 1\), we can expand the \(L\)-function \(L(s)\) inside of \(I(X,s,f)\) as a Dirichlet series:
      \begin{align*}
        I(X,s,f) &= \frac{1}{2\pi i}\int_{(a)}\L(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} \\
        &= \frac{1}{2\pi i}\int_{(a)}q(f)^{\frac{s+u}{2}}\g(s+u,f)L(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} \\
        &= \frac{1}{2\pi i}\int_{(a)}\sum_{n \ge 1}\frac{a(n)}{n^{s+u}}q(f)^{\frac{s+u}{2}}\g(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} \\
        &= \sum_{n \ge 1}\frac{1}{2\pi i}\int_{(a)}\frac{a(n)}{n^{s+u}}q(f)^{\frac{s+u}{2}}\g(s+u,f)\Phi(u)X^{u}\,\frac{du}{u} && \text{FTT} \\
        &= q(f)^{\frac{s}{2}}\g(s)\sum_{n \ge 1}\frac{a(n)}{n^{s}}\frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s)}\Phi(u)\left(\frac{\sqrt{q(f)}X}{n}\right)^{u}\,\frac{du}{u} \\
        &= q(f)^{\frac{s}{2}}\g(s)\sum_{n \ge 1}\frac{a(n)}{n^{s}}V_{s}\left(\frac{n}{\sqrt{q(f)}X}\right).
      \end{align*}
      Performing the same computation for \(I(X^{-1},1-s,\conj{f})\) and substituting in the results, we arrive at
      \begin{align*}
        \L(s) &= q(f)^{\frac{s}{2}}\g(s)\sum_{n \ge 1}\frac{a(n)}{n^{s}}V_{s}\left(\frac{n}{\sqrt{q(f)}X}\right) \\
        &+ \e(f)q(f)^{\frac{1-s}{2}}\g(1-s,f)\sum_{n \ge 1}\frac{\conj{a(n)}}{n^{1-s}}V_{1-s}\left(\frac{nX}{\sqrt{q(f)}}\right)+R(s,X,f).
      \end{align*}
      Diving by \(q(f)^{\frac{s}{2}}\g(s)\) completes the proof.
    \end{proof}
    ...
    \begin{proposition}\label{prop:V_function_decay}
      ...
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{(2M-2)}\frac{\g(s+u,f)}{\g(s)}\Phi(u)y^{-u}\frac{du}{u}.
      \]
    ...
    \begin{proof}
      ...
      \[
        \frac{\g(s+u,f)}{\g(s)} \ll \mf{q}_{\infty}(s)^{\frac{\tau}{2}}e^{d_{f}\frac{\pi}{2}|r|},
      \]
      ...
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{(M)}\frac{\g(s+u,f)}{\g(s)}\Phi(u)y^{-u}\frac{du}{u} \\
      \]
    ...
    \begin{theorem}
      ...
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{\e(f)}{\sqrt{q(f)}}\sum_{n \ge 1}\conj{a(n)}\phi(n)+R(f)\Psi(1),
      \]
      where \(\phi(y)\) is the inverse Mellin transform defined by
      \[
        \phi(y) = \frac{1}{2\pi i}\int_{(a)}q(f)^{s}\frac{\g(s)}{\g(1-s,f)}y^{-s}\Psi(1-s)\,ds,
      \]
    ...
    \begin{proof}
      ...
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{1}{2\pi i}\int_{(1-a)}\e(f)q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s)}L(1-s,\conj{f})\Psi(s)\,ds+R(f)\Psi(1).
      \]
      Performing the change of variables \(s \mapsto 1-s\) in this latter integral gives
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{1}{2\pi i}\int_{(a)}\e(f)q(f)^{s-\frac{1}{2}}\frac{\g(s)}{\g(1-s,f)}L(s,\conj{f})\Psi(1-s)\,ds+R(f)\Psi(1).
      \]
  ...
  \section{The Riemann Hypothesis and Nontrivial Zeros}
    ...
    \[
      L(s) = \prod_{p}\prod_{1 \le j \le d_{f}}(1-\a_{j}(p)p^{-s})^{-1},
    \]
    ...
    \begin{equation}\label{equ:fun_eq_for_zeros}
      L(s) = \e(f)q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s)}L(1-s,\conj{f}).
    \end{equation}
    If \(\s < 0\) then \(L(1-s,\conj{f})\) is nonzero by our previous comments. Moreover, \(\g(1-s,f)\) is holomorphic and nonzero in this region because \(\Re(\k_{j}) > -1\). We conclude that poles of \(\g(s)\) are zeros of \(L(s)\) for \(\s < 0\). Such a zero is called a \textit\{trivial zero\}. From the definition of \(\g(s)\), they are all simple and of the form \(s = -(\k_{j}+2n)\) for some local root at infinity \(\k_{j}\) and some integer \(n \ge 0\). Any other zero of \(L(s)\) is called a \textit\{nontrivial zero\} and it lies inside of the critical strip (it may also be a pole of \(\g(s)\)). Now let \(\rho\) be a nontrivial zero of \(L(s)\). Note that \(L(\conj{s},\conj{f}) = \conj{L(s)}\) for \(\s > 1\) where \(L(s)\) is defined by a Dirichlet series and thus for all \(s\) by the identity theorem. It follows that \(\conj{\rho}\) is a nontrivial zero of \(L(s,\conj{f})\) and hence, from the functional equation, \(1-\conj{\rho}\) is a nontrivial zero of \(L(s)\) too. In short, the nontrivial zeros occur in pairs:
    \[
  ...
  \section{The Lindel\"of Hypothesis and Convexity Bounds}
    ...
    \[
      L(s) = \e(f)q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s)}L(1-s,f).
    \]
    Applying \cref{equ:gamma_factor_analytic_conductor_estimate} result in the bound
    \[
      L(s) \ll_{\e} \mf{q}(s)^{\frac{1}{2}-\s}L(1-s,f),
    \]
    ...
    \]
    which holds since \(L(1-s,f)\) is defined by a locally absolutely uniformly convergent Dirichlet series for \(\s < 0\). As \(p_{r_{f}}(s)L(s)\) is holomorphic in a region containing the vertical strip \(-\e \le \s \le 1+\e\) and \(p_{r_{f}}(s) \sim 1\), the Phragm\'en-Lindel\"of convexity principle gives
  ...
  \section{Estimates on the Critical Line}
    ...
    \begin{proof}
      ...
      \begin{align*}
        L\left(\frac{1}{2}+it,f\right) &= \sum_{n \ge 1}\frac{a(n)}{n^{\frac{1}{2}+it}}V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q(f)}}\right) \\
        &+ \e\left(\frac{1}{2}+it,f\right)\sum_{n \ge 1}\frac{\conj{a(n)}}{n^{\frac{1}{2}-it}}V_{\frac{1}{2}-it}\left(\frac{n}{\sqrt{q(f)}}\right)+\frac{R\left(\frac{1}{2}+it,1,f\right)}{q(f)^{\frac{1}{4}}\g\left(\frac{1}{2}+it,f\right)}.
      \end{align*}
      ...
      \[
        L\left(\frac{1}{2}+it,f\right) \ll \left|\sum_{n \ge 1}\frac{a(n)n^{-it}}{\sqrt{n}}V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q(f)}}\right)\right|+\left|\sum_{n \ge 1}\frac{\conj{a(n)}n^{it}}{\sqrt{n}}V_{\frac{1}{2}-it}\left(\frac{n}{\sqrt{q(f)}}\right)\right|+\left|\frac{R\left(\frac{1}{2}+it,f\right)}{q(f)^{\frac{1}{4}}\g\left(\frac{1}{2}+it,f\right)}\right|.
      \]
      ...
      \[
        L\left(\frac{1}{2}+it,f\right) \ll \left|\sum_{n \ge 1}\frac{a(n)n^{-it}}{\sqrt{n}}V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q(f)}}\right)\right|+\left|\sum_{n \ge 1}\frac{\conj{a(n)}n^{it}}{\sqrt{n}}V_{\frac{1}{2}-it}\left(\frac{n}{\sqrt{q(f)}}\right)\right|.
      \]
      ...
      \begin{align*}
        \sum_{n \ge 1}\frac{a(n)n^{-it}}{\sqrt{n}}V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q(f)}}\right) &= \sum_{n \ge 1}\frac{a(n)}{\sqrt{n}}\sum_{k \in \Z}\psi_{k}\left(\frac{n}{\sqrt{q(f)}}\right)V_{\frac{1}{2}}\left(\frac{n}{\sqrt{q(f)}}\right) \\
        &= \sum_{k \in \Z}\sum_{n \ge 1}\frac{a(n)n^{-it}}{\sqrt{n}}\psi_{k}\left(\frac{n}{\sqrt{q(f)}}\right)V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q(f)}}\right) && \text{FTT} \\
        &\ll \sum_{k \in \Z}\left|\sum_{n \ll \sqrt{\mf{q}\left(\frac{1}{2}+it,f\right)}}\frac{a(n)n^{-it}}{\sqrt{n}}\psi_{k}\left(\frac{n}{\sqrt{q(f)}}\right)\right| \\
        &\ll \sum_{k \ll \log{\mf{q}_{\infty}\left(\frac{1}{2}+it\right)}}\left|\sum_{2^{k-1}\sqrt{q(f)} \le n \le 2^{k+1}\sqrt{q(f)}}\frac{a(n)n^{-it}}{\sqrt{n}}\psi_{k}\left(\frac{n}{\sqrt{q(f)}}\right)\right| \\
      ...
      \[
        \left|\sum_{n \ge 1}\frac{\conj{a(n)}n^{it}}{\sqrt{n}}V_{\frac{1}{2}-it}\left(\frac{n}{\sqrt{q(f)}}\right)\right| = \left|\sum_{n \ge 1}\frac{a(n)n^{-it}}{\sqrt{n}}\conj{V_{\frac{1}{2}-it}\left(\frac{n}{\sqrt{q(f)}}\right)}\right|,
      \]
  ...
  \section{Logarithmic Derivatives}
    ...
    \begin{proof}
      ...
      \begin{equation}\label{equ:log_derivative_1}
        \frac{\xi'}{\xi}(s) = \frac{r_{f}}{s}+\frac{r_{f}}{s-1}+\frac{1}{2}\log{q(f)}+\frac{\g'}{\g}(s)+\frac{L'}{L}(s).
      \end{equation}
    ...
    \begin{equation}\label{equ:log_derivaive_direct_computation}
      -\frac{L'}{L}(s) = -\sum_{p}\sum_{1 \le j \le d_{f}}\frac{d}{ds}\log(1-\a_{j}(p)p^{-s}) = \sum_{p}\sum_{1 \le j \le d_{f}}\frac{\a_{j}(p)\log(p)}{(1-\a_{j}(p)p^{-s})p^{s}}.
    \end{equation}
    ...
    \[
       -\frac{L'}{L}(s) = \sum_{n \ge 1}\frac{\L_{f}(n)}{n^{s}},
    \]
    for \(\s > 1\), where
    \[
  ...
  \section{Zero Density}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \begin{equation}\label{equ:log_conductor_gamma_factor_bound_analytic_conductor}
          \frac{1}{2}\log{q(f)}+\frac{\g'}{\g}(s) \ll \log{\mf{q}(s)},
        \end{equation}
    ...
    \begin{proof}
      ...
      \begin{align*}
        \D_{\eta_{L}}\arg(\xi(s)) &= \D_{\eta_{L}}\arg\left(\e(f)\xi(1-s,\conj{f})\right) \\
        &= -\D_{\conj{\eta_{R}}}\arg\left(\e(f)\xi(s,\conj{f})\right) \\
        &= \D_{\conj{\eta_{R}}}\arg\left(\conj{\e(f)}\xi(\conj{s},f)\right) \\
        &= \D_{\eta_{R}}\arg\left(\conj{\e(f)}\xi(s)\right) \\
        &= \D_{\eta_{R}}\arg\left(\conj{\e(f)}\right)+\D_{\eta_{R}}\arg(\xi(s)) \\
        &= \D_{\eta_{R}}\arg(\xi(s)).
      \end{align*}
      ...
      \[
        \xi(s) = (s(1-s))^{r_{f}}q(f)^{\frac{s}{2}}\pi^{-\frac{d_{f}s}{2}}\prod_{1 \le j \le d_{f}}\G\left(\frac{s+\k_{j}}{2}\right)L(s).
      \]
    ...
    For any \(L\)-function \(L(s)\) and \(W \ge \frac{1}{\pi}\log\left(\frac{q(f)}{(2\pi e)^{d_{f}}}\right)\),
    \[
  ...
  \section{A Zero-free Region}
    ...
    \begin{lemma}\label{lem:non-vanshing_at_1_lemma}
      Let \(L(s)\) be an \(L\)-function such that \(\Re(\L_{f}(n)) \ge 0\) provided \((n,q(f)) = 1\). Also suppose that \(|\a_{j}(p)| \le \frac{p}{2}\) for ramified primes \(p\). Then \(L(1,f) \neq 0\) and hence \(r_{f} \ge 0\). Moreover, there exists a constant \(c > 0\) such that \(L(s)\) has at most \(r_{f}\) real zeros in the region
      \[
    ...
    \begin{proof}
      ...
      \]
      To estimate \(\Re\left(\frac{L'}{L}(\s,f)\right)\), we first note that as \(\Re(\L_{f}(n)) \ge 0\) provided \((n,q(f)) = 1\) by assumption, the Dirichlet series of \(\frac{L'^{(q(f))}}{L^{(q(f))}}(s)\) shows that
      \[
    ...
    \begin{theorem}\label{thm:zero_free_region_generic}
      Let \(L(s)\) be an \(L\)-function with at most a simple pole at \(s = 1\), \(\Re(\L_{f}(n)) \ge 0\) provided \((n,q(f)) = 1\), and \(|\a_{j}(p)| \le \frac{p}{2}\) for ramified primes \(p\). Then there exists a constant \(c > 0\) such that \(L(s)\) has no zeros in the region
      \[
  ...
  \section{The Explicit Formula}
    ...
    \begin{proof}
      ...
      \[
        \sum_{n \ge 1}\L_{f}(n)\psi(n) = \frac{1}{2\pi i}\int_{(c)}-\frac{L'}{L}(s)\Phi(s)\,ds,
      \]
      ...
      \[
        \sum_{n \ge 1}\L_{f}(n)\psi(n) = r_{f}\Psi(1)-\sum_{\rho}\Psi(\rho)+\frac{1}{2\pi i}\int_{(-\d)}-\frac{L'}{L}(s)\Psi(s)\,ds.
      \]
      For the latter integral, the functional equation and that \(q(\conj{f}) = q(f)\) together imply
      \[
        -\frac{L'}{L}(s) = \log{q(f)}+\frac{\g'}{\g}(s)+\frac{\g'}{\g}(1-s,\conj{f})+\frac{L'}{L}(1-s,\conj{f}).
      \]
      Thus
      \[
        \frac{1}{2\pi i}\int_{(-\d)}-\frac{L'}{L}(s)\Phi(s)\,ds = \frac{1}{2\pi i}\int_{(-\d)}\left(\log{q(f)}+\frac{\g'}{\g}(s)+\frac{\g'}{\g}(1-s,\conj{f})+\frac{L'}{L}(1-s,\conj{f})\right)\Psi(s)\,ds.
      \]
    ...
    The explicit formula is a very useful tool for analytic investigations. Since \(\L_{f}(n)\) is essentially a weighted sum over prime powers, the explicit formula can be thought of as expressing a smoothed weighted sum over primes for \(f\) in terms of the zeros of \(L(s)\).
\fi\end{document}