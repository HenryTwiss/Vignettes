...
\begin{document}
  ...
  \section{Analytic Data}
    ...
    \[
      \G(s) \ll_{\e} (|t|+3)^{\s-\frac{1}{2}}e^{-\frac{\pi}{2}|t|} \quad \text{and} \quad \frac{1}{\G(s)} \ll_{\e} (|t|+3)^{\frac{1}{2}-\s}e^{\frac{\pi}{2}|t|},
    \]
    and hence
    \[
  ...
  \section{The Approximate Functional Equation}
    ...
    \end{proposition}
    \begin{proof}
      Observe that \((s-1)^{m_{L}}L(s) \ll_{\e} (|t|+3)^{m_{L}}\) on the vertical line \(\s = \max(1+\e,\s_{2})\). Now the functional equation and \cref{equ:gamma_factor_analytic_conductor_estimate} together give
      \[
      ...
      \]
      on the vertical line \(\s \le \min(-\e,\s_{1})\). As \((s-1)^{m_{L}}L(s)\) is entire and of order \(1\), we can apply the Phragm\'en-Lindel\"of convexity principle in this vertical strip to \((s-1)^{m_{L}}L(s)\). Whence there is a positive constant \(A'\) such that
      \[
        (s-1)^{m_{L}}L(s) \ll_{\e} (|t|+3)^{A'},
      \]
      provided \(\s_{1} \le \s \le \s_{2}\). Assuming \(s\) is at least distance \(\e\) away from the possible pole at \(s = 1\), diving by \((s-1)^{m_{L}}\) completes the proof.
    \end{proof}

    We now prove the \textit{approximate function equation}:

    \begin{theorem*}[Approximate functional equation]
      Suppose \(L(s)\) is an analytic \(L\)-function, \(\Phi(u)\) is an even holomorphic function bounded in the vertical strip \(|\tau| < a+1\) with \(a > 1\) such that \(\Phi(0) = 1\), and let \(X > 0\). Then for \(\e < \s < 1-\e\), we have
      \[
    ...
    \begin{proof}
      ...
      \]    
      Stirling's formula shows that \(L_{\infty}(s+u)\) exhibits exponential decay while \(L(s+u)\) has polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}. Since \(\Phi(u)\) is bounded in the vertical strip \(|\tau| < a+1\), it follows that the integrand exhibits exponential decay. Therefore the integral is locally absolutely uniformly bounded. We will evaluate the integral in two ways. On the one hand, we can expand \(L(s+u)\) inside the integrand as a Dirichlet series and by absolute boundedness of the integral we may interchange the sum and integral. A short computation shows that this is
      \[
    ...
    The approximate functional equation was first developed by Hardy and Littlewood in the series \cite{hardyzeros1921,hardyapproximate1923,hardyapproximate1929}. The inverse Mellin transform \(V_{s}(y)\) has the effect of dampening the two dual sums appearing on the right-hand side of the approximate functional equation. In practice, it is common to choose \(\Phi(u)\) such that \(V_{s}(y)\) is smooth with polynomial decay of arbitrarily large order. To achieve this we may take
    \[
      \Phi(u) = \cos^{-4d_{L}M}\left(\frac{\pi u}{4M}\right),
    \]
    for some positive integer \(M\). Clearly \(\Phi(u)\) is an even holomorphic function in the vertical strip \(|\tau| < (2M-1)+1\) and satisfies \(\Phi(0) = 1\). It also admits exponential decay via
    \begin{equation}\label{equ:choice_for_V_decay_estimate}
     \cos^{-4d_{L}M}\left(\frac{\pi u}{4M}\right) = \left(\frac{e^{i\frac{\pi u}{4M}}+e^{-i\frac{\pi u}{4M}}}{2}\right)^{-4d_{L}M} \ll e^{-d_{L}\pi|r|},
    \end{equation}
    which is more than sufficient. With this choice of \(\Phi(u)\), \(V_{s}(y)\) becomes smooth and satisfies useful bounds for its derivatives.
    ...
    \begin{proposition}\label{prop:V_function_decay}
      ...
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{(2M-1)}\frac{L_{\infty}(s+u)}{L_{\infty}(s)}\Phi(u)y^{-u}\frac{du}{u}.
      \]
      Then \(V_{s}(y)\) is smooth for \(\e < \s < 1-\e\). Moreover, for any nonnegative integer \(n\) and positive integer \(N\), \(V_{s}(y)\) satisfies the estimates
      \[
        (-y)^{n}V_{s}^{(n)}(y) = \begin{cases} O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{-N}\right) & \text{if \(y \gg \sqrt{\mf{q}_{\infty}(s)}\)}, \\ \d_{n,0}+O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{N}\right) & \text{if \(y \ll \sqrt{\mf{q}_{\infty}(s)}\)}. \end{cases}
      \]
      In particular,
      \[
        (-y)^{n}V_{s}^{(n)}(y) \ll_{\e} \left(1+\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{-N}.
      \]
    \end{proposition}
    \begin{proof}
      Stirling's formula implies
      \[
        \frac{\G(s+u)}{\G(s)} \ll_{\e} (|t+r|+3)^{\tau}e^{\frac{\pi}{2}|r|}.
      \]
      Whence
      \begin{equation}\label{equ:V_function_decay_1}
        \frac{L_{\infty}(s+u)}{L_{\infty}(s)} \ll_{\e} \mf{q}_{\infty}(s+u)^{\frac{\tau}{2}}e^{d_{L}\frac{\pi}{2}|r|}.
      \end{equation}
      By \cref{equ:choice_for_V_decay_estimate}, the integrand defining \(V_{s}(y)\) admits exponential decay. This forces \(V_{s}(y)\) to be locally absolutely uniformly bounded and therefore smooth. In particular, we may differentiate under the integral sign. Then
      \[
        (-y)^{n}V_{s}^{(n)}(y) = \frac{1}{2\pi i}\int_{(2M-1)}\frac{L_{\infty}(s+u)}{L_{\infty}(s)}\Phi(u)(u)_{n}y^{-u}\frac{du}{u}.
      \]
      The exponential decay also permits us to shift the line of integration. Shifting to \((N)\), we do not pass by any poles and obtain
      \[
        (-y)^{n}V_{s}^{(n)}(y) = \frac{1}{2\pi i}\int_{(N)}\frac{L_{\infty}(s+u)}{L_{\infty}(s)}\Phi(u)(u)_{n}y^{-u}\frac{du}{u}.
      \]
      Apply \cref{equ:choice_for_V_decay_estimate,equ:V_function_decay_1} to deduce
      \[
        (-y)^{n}V_{s}^{(n)}(y) \ll_{\e} \int_{(N)}\mf{q}_{\infty}(s+u)^{\frac{\tau}{2}}e^{-d_{L}\frac{\pi}{2}|r|}u^{n}y^{-u}\,\frac{du}{u}.
      \]
      By the exponential decay, we may assume \(r \ll 1\) in estimating the integral. The bound
      \[
        (-y)^{n}V_{s}^{(n)}(y) = O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{-N}\right),
      \]
      follows at once. If we instead shift to \((-N)\), we pass by a simple pole at \(u = 0\) of residue \(1\) if and only if \(n = 0\). This gives
      \[
        (-y)^{n}V_{s}^{(n)}(y) = \d_{n,0}+\frac{1}{2\pi i}\int_{(-N)}\frac{L_{\infty}(s+u)}{L_{\infty}(s)}\Phi(u)(u)_{n}y^{-u}\frac{du}{u}.
      \]
      An analogous argument to estimate the remaining integral shows
      \[
        (-y)^{n}V_{s}^{(n)}(y) = \d_{n,0}+O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{N}\right).
      \]
      These \(O\)-estimates give nontrivial bounds for \(y \gg \sqrt{\mf{q}_{\infty}(s)}\) and \(y \ll \sqrt{\mf{q}_{\infty}(s)}\) respectively. Upon combining both, we obtain
      \[
        (-y)^{n}V_{s}^{(n)}(y) \ll_{\e} \left(1+\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{-N}.
      \]
    \end{proof}

    With our choice of \(\Phi(u)\), these estimates show \(V_{s}(y)\) is bounded for \(y \ll \sqrt{\mf{q}_{\infty}(s)}\) and exhibits polynomial decay of arbitrarily large order thereafter.
    
    In a similar spirit to the approximate functional equation, a useful summation formula can be derived from the functional equation.

    \begin{theorem}
      Let \(L(s)\) be an analytic \(L\)-function and let \(\psi(y)\) be a bump function whose support is bounded away from zero where \(\Psi(s)\) is its Mellin transform. Then
      \[
    ...
    \begin{proof}
      ...
      \]
      By our choice of \(\psi(y)\), its inverse Mellin transform \(\Psi(s)\) has arbitrarily large polynomial decay. Since \(L(s)\) has polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}, the integrand has polynomial decay of arbitrarily larger order. Therefore is locally absolutely uniformly bounded which permits us to shift the line of integration. Shifting to \((1-a)\), we pass by a potential pole at \(s = 1\) and obtain
      \[
      ...
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{1}{2\pi i}\int_{(1-a)}\e_{L}q_{L}^{\frac{1}{2}-s}\frac{L_{\infty}(1-s)}{L_{\infty}(s)}\conj{L(1-\conj{s})}\Psi(s)\,ds+R(f)\Psi(1).
      \]
      Performing the change of variables \(s \mapsto 1-s\) in this latter integral gives
      \[
        \sum_{n \ge 1}a(n)\psi(n) = \frac{1}{2\pi i}\int_{(a)}\e_{L}q_{L}^{s-\frac{1}{2}}\frac{L_{\infty}(s)}{L_{\infty}(1-s)}\conj{L(\conj{s})}\Psi(1-s)\,ds+R(f)\Psi(1).
      \]
  ...
  \section{Estimates on the Critical Line}
    ...
    \end{theorem}
    \begin{proof}
      Taking \(s = \frac{1}{2}+it\), \(X = 1\), and \(\Phi(u) = \cos^{-4d_{L}M}\left(\frac{\pi u}{4M}\right)\) for some integer \(M \ge 1\) in the approximate functional equation gives
      \begin{align*}
  ...
  \section{Zero Density}
    ...
    \begin{proof}
      ...
      \[
        \xi(s) = (s(1-s))^{r_{f}}Q^{\frac{s}{2}}\pi^{-\frac{d_{L}s}{2}}\prod_{j}\G\left(\frac{s+\k_{j}}{2}\right)L(s).
      \]
      ...
      \begin{equation}\label{equ:zero_counting_4}
        \D_{\eta_{L}}\arg\left(\prod_{j}\G\left(\frac{s+\k_{j}}{2}\right)\right) = \frac{T}{2}\log\left(\frac{T^{d_{L}}}{(2e)^{d_{L}}}\right)+O(\log{\mf{q}(f)}).
      \end{equation}
      ...
      \[
        \frac{1}{\pi}\D_{\eta_{L}}\arg\xi(s) = \frac{T}{2\pi}\log\left(\frac{T^{d_{L}}}{(2\pi e)^{d_{L}}}\right)+O(\log{\mf{q}(iT,f)}),
      \]
      and therefore
      \[
    ...
    For any \(L\)-function \(L(s)\) and \(W \ge \frac{1}{\pi}\log\left(\frac{Q}{(2\pi e)^{d_{L}}}\right)\),
    \[
  ...
  \section{A Zero-free Region}
    ...
    \begin{lemma}\label{lem:non-vanshing_at_1_lemma}
      ...
      \[
        \s \ge 1-\frac{c}{d_{L}(r_{f}+1)\log{\mf{q}(f)}}.
      \]
    \end{lemma}
    \begin{proof}
      ...
      \[
        \Re\left(\frac{L'_{Q}}{L_{Q}}(\s,f)\right) \le \left|\frac{L'_{Q}}{L_{Q}}(\s,f)\right| = \left|\sum_{p \mid Q}\sum_{j}\frac{\a_{j}(p)\log(p)}{(1-\a_{j}(p)p^{-\s})p^{\s}}\right| \le d_{L}\sum_{p \mid Q}\log(p) \le d_{L}\log{Q},
      \]
      ...
      \[
        \sum_{j}\frac{1}{\s-\b_{j}} < \frac{r_{f}}{\s-1}+O(\log{d_{L}\mf{q}(f)}).
      \]
      ...
      \[
        \b_{j} \ge 1-\frac{c}{d_{L}(r_{f}+1)\log{\mf{q}(f)}}.
      \]
      ...
      \[
        \frac{nd_{L}\log{\mf{q}(f)}}{2c+\frac{c}{(r_{f}+1)}} < \left(\frac{r_{f}}{2c}+O(1)\right)d_{L}\log{\mf{q}(f)}.
      \]
