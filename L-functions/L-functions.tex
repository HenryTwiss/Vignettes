...
\begin{document}
  ...
  \section{The Approximate Functional Equation}
    ...
    The approximate functional equation was first developed by Hardy and Littlewood in the series \cite{hardyzeros1921,hardyapproximate1923,hardyapproximate1929}. The inverse Mellin transform \(V_{s}(y)\) has the effect of smoothing the two dual sums appearing on the right-hand side of the approximate functional equation. In practice, it is common to take
    \[
      \Phi(u) = \cos^{-4d_{L}M}\left(\frac{\pi u}{4M}\right),
    \]
    for some positive integer \(M\). Clearly \(\Phi(u)\) is an even holomorphic function in the vertical strip \(|\tau| < (2M-2)+1\) and satisfies \(\Phi(0) = 1\). To see that it is bounded in this vertical strip, simply observe that it admits exponential decay
    ...
    \end{equation}
    which is more than sufficient. With this choice of \( \Phi(u)\), it is possible to derive a useful bound for the inverse Mellin transform \(V_{s}(y)\).
    ...
    \begin{proposition}\label{prop:V_function_decay}
      ...
      \]
      Then for \(\e < \s < 1-\e\), \(V_{s}(y)\) satisfies the estimate
      \[
        V_{s}(y) \ll_{\e} \left(1+\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{-M}.
      \]
    \end{proposition}
    \iffalse\begin{proof}
      ...
      Whence
      \[
        \frac{L_{\infty}(s+u)}{L_{\infty}(s)} \ll_{\e} \mf{q}_{\infty}(s+u)^{\tau}e^{d_{L}\frac{\pi}{2}|r|}.
      \]
      By \cref{equ:choice_for_V_decay_estimate}, the integral defining \(V_{s}(y)\) is locally absolutely uniformly convergent and we may shift the line of integration to \((M)\). In doing so, we do not pass by any poles and obtain
      \[
      ...
      \begin{align*}
        V_{s}(y) &= \frac{1}{2\pi i}\int_{(M)}\frac{L_{\infty}(s+u)}{L_{\infty}(s)}\Phi(u)y^{-u}\frac{du}{u} \\
        &\ll \int_{-\infty}^{\infty}\mf{q}_{\infty}(s)^{\frac{M}{2}}e^{-d_{L}\frac{\pi}{2}|r|}y^{-M}\,dr \\
        &\ll \int_{-\infty}^{\infty}\mf{q}_{\infty}(s)^{\frac{M}{2}}e^{-d_{L}\frac{\pi}{2}|r|}(1+y)^{-M}\,dr \\
        &\ll \int_{-\infty}^{\infty}e^{-\frac{\pi}{2}d_{L}|r|}\left(1+\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{-M}\,dr \\
        &\ll \left(1+\frac{y}{\sqrt{\mf{q}_{\infty}(s)}}\right)^{-M}\int_{-\infty}^{\infty}e^{-\frac{\pi}{2}d_{L}|r|}\,dr \\
