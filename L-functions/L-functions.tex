...
\begin{document}
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{theorem*}[Approximate functional equation]
      Suppose \(L(s)\) is an analytic \(L\)-function, \(\Phi(u)\) be an even holomorphic function bounded in the vertical strip \(|\tau| < a+1\) with \(a > 1\) such that \(\Phi(0) = 1\), and let \(X > 0\). Then for \(\e < \s < 1-\e\), we have
      \[
