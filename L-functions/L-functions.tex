...
\begin{document}
  ...
  \section{Analytic Data}
    ...
    \]
    with coefficients \(a(n) \in \C\) such that \(a(1) = 1\) and satisfying the following properties:
    \begin{enumerate}[align=left]
      \item[\textbf{Analyticity:}] There exists a nonnegative integer \(r\) such that the Dirichlet series of \(L(s)\) is absolutely convergent for \(\s > 1\) and admits meromorphic continuation to \(\C\) with at most a pole at \(s = 1\) of order \(r\). Moreover, \((s-1)^{r}L(s)\) is order \(1\).
      \item[\textbf{Functional Equation:}] There is a positive integer \(Q\), called the \textit{conductor} of \(L(s)\), a positive integer \(d\) called the \textit{degree} of \(L(s)\), complex numbers \((\mu_{j})_{1 \le j \le d}\) called the \textit{local parameters} of \(L(s)\) at infinity that are either real or occur in conjugate pairs and satisfy \(\Re(\mu_{j}) \ge 0\), and a complex number \(\e\) called the \textit{root number} of \(L(s)\) with \(|\e| = 1\), all of which determine functions
      \[
        \L(s) = Q^{\frac{s}{2}}L_{\infty}(s)L(s) \quad \text{and} \quad L_{\infty}(s) = \pi^{-\frac{ds}{2}}\prod_{j}\G\left(\frac{s+\mu_{j}}{2}\right),
      \]
      ...
      \]
      This is called the \textit{functional equation} of \(L(s)\).
      \item[\textbf{Euler product:}] For every prime \(p\), there exist complex numbers \((\a_{j}(p))_{1 \le j \le d}\) called the \textit{local parameters} of \(L(s)\) at \(p\) satisfying \(|\a_{j}(p)| < p\) and are such that \(L(s)\) admits the Euler product
      \[
        L(s) = \prod_{p}(1-\a_{1}(p)p^{-s})^{-1} \cdots (1-\a_{d}(p)p^{-s})^{-1},
      \]
      which is absolutely convergent for \(\s > 1\).
    \end{enumerate}
    ...
    \begin{enumerate}[align=left]
      ...
      \item[\textbf{Ramanujan bound:}] The local parameters at \(p\) satisfy \(|\a_{j}(p)| = 1\) for \(p \nmid Q\) and \(|\a_{j}(p)| \le 1\) for \(p \mid Q\).
    \end{enumerate}

    Some comments on the definition of analytic \(L\)-functions are in order. The bound \(\Re(\mu_{j}) \ge 0\) ensures that the local factor at infinity is holomorphic in the region \(\s > 0\). As this factor is then guaranteed to be finite and nonzero at \(s = 1\), the completion possesses a pole at \(s = 1\) of order \(r\). By the functional equation, the completion also has a pole at \(s = 0\) of the same order. Somewhat less obvious is the fact that we may merely assume \((s-1)^{r}L(s)\) is finite order as the functional equation forces the order to be \(1\). This can be deduced by considering the entire function
    \[
      s^{r}(s-1)^{r}\L(s),
    \]
    and applying the Phragm\'en-Lindel\"of convexity principle in vertical strips. Also, the condition that the root number satisfies \(|\e| = 1\) isn't strictly necessary. For applying the functional equation twice shows \(|\e|^{2} = 1\). The Selberg and Ramanujan bounds are conjectural for most analytic \(L\)-functions, but the latter is the more important bound. The Euler product implies that the coefficients \(a(n)\) are multiplicative and are determined on prime powers by
    \[
      a(p^{k}) = \sum_{1 \le j_{1} \le \cdots \le j_{k} \le d}\a_{j_{1}}(p) \cdots \a_{j_{k}}(p).
    \]
    In other words, \(a(p^{k})\) is the complete symmetric polynomial of degree \(k\) in the local parameters at \(p\). There are \(\binom{k+d-1}{d-1} \ll \s_{d}(p^{k})\) many terms and the coefficients are bounded by \(a(n) \ll \s_{d}(n)\). This implies the slightly weaker, but more practical, estimate \(a(n) \ll_{\e} n^{\e}\).

    It it clear from the definition that analytic \(L\)-functions are closed under multiplication and that the degree is additive. Moreover, this closure respects unitary \(L\)-functions as well. This makes the set of analytic \(L\)-functions into an well-founded commutative semigroup where the partial order induced by degree. Then there exist irreducible elements with respect to the partial order and we say that an analytic \(L\)-function is \textit{primitive} if it such an irreducible. Clearly any analytic \(L\)-function of degree \(1\) is primitive. Moreover, every analytic \(L\)-function factors into a product of primitive \(L\)-functions. Such a factorization is only conjectured to be unique for unitary \(L\)-functions. A sufficient condition would be \textit{Selberg's orthogonality conjecture}:

    \begin{conjecture*}[Selberg's orthogonality conjecture]
      For any two primitive unitary \(L\)-functions \(L_{1}\) and \(L_{2}\) with coefficients \(a_{1}(n)\) and \(a_{2}(n)\), we have
      \[
        \sum_{p \le x}\frac{a_{1}(p)\conj{a_{2}(p)}}{p} = \d_{L_{1},L_{2}}\log\log(x)+O(1).
      \]
    \end{conjecture*}
    ...
    Lastly, we provide some bounds about gamma functions, gamma factors, and their logarithmic derivatives that will be extremely useful in the study of \(L\)-functions. 


    To each analytic \(L\)-function, we also associate its \textit{analytic conductor} \(\mf{q}(s)\) defined by
    \[
      \mf{q}(s) = Q\mf{q}_{\infty}(s),
    \]
    where
    \[
      \mf{q}_{\infty}(s) = \prod_{j}(|s+\k_{j}|+3)\prod_{k}(|s+\mu_{k}|+3).
    \]
    ...
    Suppose \(\s\) is bounded and \(|t| > 1\). Then \(s\) is bounded away from zero and by \cref{equ:weaker_Stirling_formula} we have the asymptotic
    \[
