...
\begin{document}
  \date{}
  \maketitle
  Throughout \(s = \s+it\) and \(s_{0} = \s_{0}+it_{0}\) will stand for a complex variable with \(\s,\s_{0},t,t_{0}\) real
  \section{Convergence Properties}
    ...
    \begin{theorem}\label{thm:convergence_of_Dirichlet_series}
      ...
      \]
      In particular, \(D(s)\) converges in the region \(\s > \s_{0}\).
    \end{theorem}
    \begin{proof}
    ...
    \end{proposition}
    \begin{proof}
      Necessarily \(\s_{a} \le \a\) and so \(D(s)\) converges absolutely in the region \(\s > \a\). Write
      \[
    ...
    It turns out that if \(A(X)\) is bounded independent of \(X\), then \(\s_{c}\) is negative.
    ...
    Unfortunately, it is not often the case that \(A(X)\) is bounded independent of \(X\) as this is quite a strong condition for most Dirichlet series. Fortunately, we can still obtain nice result analogous to that of \cref{prop:Dirichlet_series_convergence_polynomial_bound} if we assume \(|A|(X)\) grows at most polynomially in \(X\).
    ...
    \begin{proof}
      ...
      where
      \[
        c_{k} = \frac{1}{k!}\sum_{n \ge 1}\frac{a(n)(-\log(n))^{k}}{n},
      \]
      ...
      \[
        P(s) = \sum_{k \ge 0}\frac{(1-s)^{k}}{k!}\sum_{n \ge 1}\frac{a(n)(\log(n))^{k}}{n}.
      \]
  ...
  \section{Euler Products}
    Generally speaking, if the coefficients \(a(n)\) are chosen at random, \(D(s)\) is not guaranteed to good properties outside of convergence in some region (provided it converges at a point). However, many Dirichlet series of interest will have coefficients that are multiplicative. These Dirichlet series admits product expressions.
    ...
    \begin{proposition}\label{prop:Dirichlet_series_Euler_product}
        ...
        \]
        for \(\s > \s_{a}\), where the product is locally absolutely uniformly convergent. If the prime power coefficients satisfy
        \[
          a(p^{m}) = \sum_{1 \le j_{1} \le \cdots \le j_{m} \le d}\a_{j_{1}}(p) \cdots \a_{j_{m}}(p),
        \]
        for some \(\a_{1}(p),\ldots,\a_{d}(p) \in \C\) and positive integer \(d\), then the product takes the form
        \[
          D(s) = \prod_{p}\prod_{1 \le j \le d}(1-\a_{j}(p)p^{-s})^{-1},
        \]
    \end{proposition}
    \begin{proof}
        ...
        \]
        This series converges absolutely because \(D(s)\) does. Now let \(N \ge 1\). By absolute convergence and the fundamental theorem of arithmetic,
        \[
          \prod_{p \le N}\left(\sum_{k \ge 0}\frac{a(p^{k})}{p^{ks}}\right) = \sum_{n \le N}\frac{a(n)}{n^{s}}+\asum_{n > N}\frac{a(n)}{n^{s}},
        \]
        where the \(\ast\) indicates that we are summing over only those additional terms \(\frac{a(n)}{n^{s}}\) that appear in the expanded product. As \(N \to \infty\), the first sum on the right-hand side tends to \(D(s)\) and the second sum tends to zero because it is part of the tail of \(D(s)\). This proves that the product converges, and is equal to \(D(s)\). Local absolute uniform convergence of the product follows from that of the Dirichlet series. If the prime power coefficients are of the form
        \[
          a(p^{m}) = \sum_{1 \le j_{1} \le \cdots \le j_{m} \le d}\a_{j_{1}}(p) \cdots \a_{j_{m}}(p),
        \]
        then
        \[
          \sum_{k \ge 0}\frac{a(p^{k})}{p^{ks}} = \prod_{1 \le j \le d}(1-\a_{j}(p)p^{-s})^{-1},
        \]
        from which the product formula follows.
    \end{proof}

    In the special case of complete multiplicative coefficients, \cref{prop:Dirichlet_series_Euler_product} implies
    \[
      D(s) = \prod_{p}(1-a(p)p^{-s})^{-1}.
    \]
    More generally, suppose \(D(s)\) can be expressed as the product
    \[
      D(s) = \prod_{p}\prod_{1 \le i \le d}(1-\a_{i}(p)p^{-s})^{-1}.
    \]
    We call this product the \textit{Euler product} of \(D(s)\) and it is said to be of \textit{degree} \(d\). As in \cref{prop:Dirichlet_series_Euler_product}, the condition
    \[
      a(p^{m}) = \sum_{1 \le j_{1} \le \cdots \le j_{m} \le d}\a_{j_{1}}(p) \cdots \a_{j_{m}}(p),
    \]
    guarantees that the Dirichlet series has an Euler product of degree \(d\). In this case, we let \(D^{(N)}(s)\) denote the Dirichlet series with the factors \(p \mid N\) in the Euler product removed. This means
    \[
      D^{(N)}(s) = D(s)\prod_{p \mid N}\prod_{1 \le i \le d}(1-\a_{i}(p)p^{-s}).
    \]
  ...
  \section{Perron Formulas}
    ...
    Classical Perron's formula is not always useful for applications because often it is necessary to estimate the integral of the Dirichlet series. As the integral need not be absolutely bounded, this would require nontrivial estimates for the Dirichlet series in vertical strips. There are two ways to correct for this defect each of which leads to a different variant of Perron's formula. The first is to truncate the integral while the latter is to introduce a smoothing function. Both variants can be equally useful and the choice of which to use is often dependent upon what methods and estimates are available in the current setting.
    ...
    \]
    defined for \(y > 0\). Note that \(I(y,T)\) is just \(d(y)\) truncated outside height \(T\). The lemma we require gives an approximation for how close \(I(y,T)\) is to \(\d(y)\) and also computes \(\d(y)\) exactly. It is quite laborious but standard.
    ...
    \begin{proof}
      ...
      \[
        I(y,T) = O(y^{c}).
      \]
      Combining these two estimates for \(I(y,T)\) proves everything when \(y < 1\). Now suppose \(y > 1\) and let \(d < 0\). We argue as in the case \(y < 1\) except we use the the rectangular contour
      ...
      \]
      and so \(\d(y) = 1\). We now modify the contour by using the semicircular one in \cref{fig:discontinuous_integral_contour_4}. 
      ...
      Again, the contour encloses the simple pole of the integrand at $s = 0$ whose residue is $1$. Arguing analogously, we obtain
      \[
        I(y,T) = 1+O(y^{c}).
      \]
    ...
    Having discussed truncated Perron's formula, we turn to the variant where we instead introduce a smoothing function. Let \(\psi\) be bump function whose compact support is bounded away from zero. For any \(X > 0\), we set
    \[
      A_{\psi}(X) = \sum_{n \ge 1}a(n)\psi\left(\frac{n}{X}\right),
    \]
    We distinguish between two cases. The first is when we choose \(\psi\) to assigns weight \(1\) or \(0\) to the coefficients and we obtain sums such as
    \[
      \sum_{\frac{X}{2} \le n < X}a(n) \quad \text{or} \quad \sum_{X \le n < X+Y}a(n).
    \]
    Sums of this type are called \textit{unweighted}. As an example of an unweighted sum, let \(\psi\) be a bump function that is identically \(1\) on \(\left[\frac{1}{2},1\right]\) and is supported in \(\left[\frac{1}{2},\frac{X+1}{X}\right]\). For example, take
    \[
      \psi(y) = \begin{cases} 1 & \text{if \(\frac{1}{2} \le y \le 1\)}, \\ e^{\frac{1-y}{\frac{X+1}{X}-y}} & \text{if \(1 \le y < \frac{X+1}{X}\)}, \\ 0 & \text{if \(0 \le y < \frac{1}{2}\) or \(y \ge \frac{X+1}{X}\)}. \end{cases}
    \]
    Then
    \[
      A_{\psi}(X) = \sum_{\frac{X}{2} \le n \le X}a(n).
    \]
    In the second case, we want \(\psi\) to dampen the coefficients with a weight other than \(1\) or \(0\). Sums of this type are called \textit{weighted}. In either case, the Mellin transform \(\Psi\) of \(\psi\) is given by
    \[
      \Psi(s) = \int_{0}^{\infty}\psi(y)y^{s}\,\frac{dy}{y}.
    \]
    As \(\psi\) has compact support, \(\Psi\) defines a locally absolutely uniformly convergent function on \(\C\). In particular it is holomorphic, and the Mellin inversion formula implies that \(\psi\) is the Mellin inverse of \(\Psi\). As for nice properties, \(\Psi(s)\) exhibit polynomial decay of arbitrarily large order in vertical strips.

    \begin{proposition}\label{prop:smoothing_function_Mellin_inverse_vertical_strips}
      Suppose \(\psi\) is a bump function whose compact support is bounded away from zero and denote its Mellin transform by \(\Psi\). Then for \(\s_{1} < \s < \s_{2}\), we have
      \[
        \Psi(s) \ll_{\s_{1},\s_{2}} (|s|+1)^{-N},
      \]
      for any positive integer \(N\).
    \end{proposition}
    \begin{proof}
      Let \(\s_{1} < \s < \s_{2}\) and consider
      \[
        \Psi(s) = \int_{0}^{\infty}\psi(y)y^{s}\,\frac{dy}{y}.
      \]
      Since \(\psi\) is compactly supported, integration by parts yields
      \[
      ...
      \]
      Now \(\psi^{N}\) is compactly supported in the same region as \(\psi\). In particular, \(\psi^{(N)}(y)\) has compact support away from zero. Therefore
      \[
        \int_{0}^{\infty}\psi^{(N)}(y)y^{\s+N}\,\frac{dy}{y} \ll_{\s_{1},\s_{2}} 1,
      \]
    ...
    \begin{theorem*}[Perron's formula, smoothed]
      Let \(D(s)\) be a Dirichlet series with coefficient \(a(n)\) and finite and nonnegative abscissa of absolute convergence \(\s_{a}\). Let \(\psi\) be a bump function whose compact support is bounded away from zero and denote its Mellin transform by \(\Psi\). Then for any \(c > \s_{a}\), we have
      \[
